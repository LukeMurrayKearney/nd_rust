...
def mcmc(data, days, partitions, contact_matrix, network_params, outbreak_params, tau_0=0.02, p_hosp=0.01, iters=10_000, dist_type='nbinom', n=20_000, prior_param=5):
    partitions = [int(a) for a in partitions]
    return nd_r.mcmc_data(data=data, days=days, tau_0=tau_0, proportion_hosp=p_hosp, iters=iters, dist_type=dist_type, n=n, partitions=partitions, contact_matrix=contact_matrix, network_params=network_params, outbreak_params=outbreak_params, prior_param=prior_param)
...
def fit_to_r0(partitions, contact_matrix, r0=3, network_params=None, iterations=30, n=30_000, dist_type='nbinom', inv_gamma=4, prop_infec=1e-3, num_networks=30, num_restarts=30, scaling="None"):
    ...
    if dist_type == 'sbm':
        network_params = []
    return nd_r.test_r0_fit(n=n, partitions=partitions, dist_type=dist_type, network_params=network_params, contact_matrix=contact_matrix, parameters=outbreak_parameters, prop_infec=prop_infec, num_networks=num_networks, target_r0=r0, iters=iterations, num_replays=num_restarts, scaling=scaling)
...
def simulate(partitions, contact_matrix, network_params=None, tau=0.05, iterations=50, n=30_000, dist_type='nbinom', maxtime=10_000, inv_gamma=4, prop_infec=1e-3, scaling="None"):
    ...
    return nd_r.infection_sims(iters=iterations, n=n, partitions=partitions, dist_type=dist_type, params=network_params, contact_matrix=contact_matrix, parameters=parameters, maxtime=maxtime, prop_infec=prop_infec, scaling=scaling)
